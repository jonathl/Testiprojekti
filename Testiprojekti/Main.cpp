...
#pragma endregion
	...
	void setVAO(bool tex3D = false) {
		...
		float* texcoord;
		if(!tex3D)
			texcoord = new float[size * 2]; //‰l‰ tee aina uusia //vaik no tuhoan noi kyl tuol lopuks
		else
	...
	void mainLoop() {
		...
		for(int i = 0; i < VAO.size(); ++i)
		{
			...
			glBindTexture(GL_TEXTURE_2D, tex[i]); //texturit voi varmaan asettaa eri j‰rjestykses //TODO tekstuurit on ihan v‰‰rin mut ei sill‰ niin mit‰‰n
			//glBindTexture(GL_TEXTURE_3D, tex[i]); //texturit voi varmaan asettaa eri j‰rjestykses
