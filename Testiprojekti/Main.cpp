...
void m_drawToTexture(GLuint tex,float *texture, int width, int height) { //korjaa reunan yli piirt‰minen ja textuurin ko-on vaikuttaminen
...
void m_colorAdjacent(float *color, float *fillcolor, int texel, float *texture, int width, int height) { //poista recursio, stackki t‰yttyy
...
void m_bucketToolTexture(GLuint tex, float *texture, int width, int height) {
	...
	int index = m_getTextureCoordinate(width, height, (int)cursor_xpos, (int)cursor_ypos) * 3;
	float color[] = { texture[index], texture[index+1], texture[index+2] };// onnistuu ylitt‰m‰‰n max arvon??
